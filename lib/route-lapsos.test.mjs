...
test("does not count records with invalid or incomplete evidence as registered pairs", () => {
  assert.deepEqual(
    getCompleteRegisteredPairs([
      { establishmentId: 10, productId: 1, evidenceNum: -1, evidenceCount: 0 },
...
test("counts records with zero required evidence and zero uploaded evidence as registered pairs", () => {
  assert.deepEqual(
    getCompleteRegisteredPairs([
      { establishmentId: 10, productId: 1, evidenceNum: 0, evidenceCount: 0 },
      { establishmentId: 11, productId: 2, evidenceNum: 1, evidenceCount: 0 },
    ]),
    [{ establishmentId: 10, productId: 1 }],
  );
});
...
test("active route lapso query excludes future lapsos", () => {
  const source = readFileSync("lib/route-lapsos.ts", "utf8");
  const resolveStart = source.indexOf("export async function resolveActiveLapso");
  const resolveEnd = source.indexOf("export function getDurationDaysFromVisitPeriod", resolveStart);
  const resolveSource = source.slice(resolveStart, resolveEnd);
  const activeLookups = resolveSource.match(/\.eq\("status", "en_curso"\)[\s\S]*?\.gt\("end_at", nowIso\)/g) ?? [];

  assert.ok(activeLookups.length > 0);
  for (const lookup of activeLookups) {
    assert.match(lookup, /\.lte\("start_at", nowIso\)/);
    assert.doesNotMatch(lookup, /\.gte\("start_at", currentWeekStartIso\)/);
  }
});
